...
let ticketPriceTotal = runners.reduce((accumulator, value) => accumulator + value.donation, 0);
...
// Problem 1  ------------------------------------------>
// We forgot to create a property for the distance each runner is going to run and that information needs to be entered asap. This info needs to be included on the runners bibs since it would determine the bib's color.
// We are given a large data set that includes all of the info about each runner, uncluding the distance a runner signed up to run.
// Let's say we are given the data array that looks like this:
const data = [
    [1, "Charmain", "Seiler", "cseiler0@wired.com", "2XL", "Divanoodle", 75, '5K'], 
    ['...'],
    ['...']
]
//save data in object for easy access
let storage = {};
data.forEach(item => storage[item[0]] = item[item.length - 1]);
//add distance property to runners array
runners.forEach(item => item['distance'] = storage[item.id]);
//add distance value to names array
runners.forEach(element => fullName.push(element.first_name + " " + element.last_name + " " + element.distance));
console.log(runners);
// <-----------------------------------------------------------

// Problem 2  ----------------------------------------------->
// Company 'Skinix' contacted saying they want to add to the donations their employees made. They will be sending a check. We need to add $50 to each of their employees donations.
runners.forEach(item => {
    if (item.company_name === "Skinix") {
        item.donation += 50;
    }
})
ticketPriceTotal = runners.reduce((accumulator, value) => accumulator + value.donation, 0);
console.log(runners)
console.log(ticketPriceTotal);
// <----------------------------------------------------------
...
// Problem 3  -------------------------------------------->
// Companies Kanoodle and Oozz are going to become sponsores and we need to get all the info of their employee runners asap.
let sponsorCompanyRunners = runners.filter(item => {
    return item.company_name === "Kanoodle" || item.company_name === "Oozz";
})
console.log(sponsorCompanyRunners);